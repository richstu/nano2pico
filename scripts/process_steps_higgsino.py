...
def processSteps(process_commands, YEAR, PRODUCTION_NAME, STEP_FILEBASENAME, PICO_DIR, NANOAOD_VERSION, FIRST_COMMAND, NO_RUN, tag):
  ...
  for iRemainStep in range(len(process_commands)-next_step):
    ...
    if error != 0:
      ...
      os.system(notify_script+' "There was an error on step '+str(next_step)+' for '+tag+'"')
      sys.exit(1)
...
def processData(YEAR, PRODUCTION_NAME, STEP_FILEBASENAME, PICO_DIR, NANOAOD_VERSION, FIRST_COMMAND, notify_script):
  ...
  process_commands = [
    ...
    './scripts/write_slim_and_merge_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/data/skim_met150/ --slim_name higmc --tag '+data_tag,
    'auto_submit_jobs.py '+data_tag+'_slim_higmc_met150_cmds.json -f',
    './scripts/confirm_slim.py '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/data/skim_met150 '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/data/merged_higmc_met150',
    notify_script+' "Finished merge met150 '+data_tag+'"'],
    
    #3
    [notify_script+' "Started skim preselect '+data_tag+'"',
    './scripts/write_skim_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/data/merged_higmc_met150 --skim_name preselect --tag '+data_tag,
    ...
    './scripts/write_skim_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/data/merged_higmc_met150 --skim_name higloose --tag '+data_tag,
