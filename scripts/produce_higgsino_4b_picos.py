...
def processData(YEAR, PRODUCTION_NAME, STEP_FILEBASENAME, LOG_FILENAME, PICO_DIR, NANOAOD_VERSION, FIRST_COMMAND, notify_script):
  ...
  process_commands = [
    ...
    './scripts/write_process_nano_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/nano/'+YEAR+'/data/ --production '+PRODUCTION_NAME+' --dataset_list txt/datasets/'+NANOAOD_VERSION+'_higgsino4b_'+YEAR+'_data_dataset_paths --data --tag '+data_tag,
...
#    notify_script+' "Finished merge llg '+data_tag+'"'],
    ...
    #3
    [notify_script+' "Started skim higlep1T '+data_tag+'"',
    ...
    [notify_script+' "Started skim higlep2T '+data_tag+'"',
...
''', formatter_class=argparse.RawTextHelpFormatter)
  ...
  for year in years:
   # FIRST_COMMAND = processMc(YEAR=year, PRODUCTION_NAME=PRODUCTION_NAME, STEP_FILEBASENAME=PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/produce_bbgg_higgsino_picos.py.'+PRODUCTION_NAME+ '.' + str(year) + '.mc.step', LOG_FILENAME= PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/produce_bbgg_higgsino_picos.py.'+PRODUCTION_NAME+ '.' + str(year) + '.mc.log', PICO_DIR=PICO_DIR, NANOAOD_VERSION=NANOAOD_VERSION, FIRST_COMMAND=FIRST_COMMAND, notify_script=notify_script)
    FIRST_COMMAND = processData(YEAR=year, PRODUCTION_NAME=PRODUCTION_NAME, STEP_FILEBASENAME=PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/produce_higgsino_4b_picos.py.'+PRODUCTION_NAME+'.' + str(year) + '.mc.step', LOG_FILENAME= PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/produce_higgsino_4b_picos.py.'+PRODUCTION_NAME+'.' + str(year) + '.mc.log', PICO_DIR=PICO_DIR, NANOAOD_VERSION=NANOAOD_VERSION, FIRST_COMMAND=FIRST_COMMAND, notify_script=notify_script)
  #for year in years:
