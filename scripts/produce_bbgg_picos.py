...
def processMc(YEAR, PRODUCTION_NAME, STEP_FILEBASENAME, LOG_FILENAME, PICO_DIR, NANOAOD_VERSION, FIRST_COMMAND, notify_script):
  ...
  process_commands = [
    ...
    './scripts/write_process_nano_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/nano/'+YEAR+'/higgsino_signal_mc/ --production '+PRODUCTION_NAME+' --dataset_list txt/datasets/'+NANOAOD_VERSION+'_bbgg_higgsino_'+YEAR+'_mc_dataset_paths --tag '+mc_tag,
    ...
    './scripts/merge_corrections.py --wgt_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/higgsino_signal_mc/wgt_sums/ --corr_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/mc/corrections/',
    notify_script+' "Finished merge corrections '+mc_tag+'"'],
    
    #2
    [notify_script+' "Start applied corrections '+mc_tag+'"',
    './scripts/write_apply_corrections_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/'+PRODUCTION_NAME+'/'+YEAR+'/higgsino_signal_mc/raw_pico/ --tag '+mc_tag,
...
def processData(YEAR, PRODUCTION_NAME, STEP_FILEBASENAME, LOG_FILENAME, PICO_DIR, NANOAOD_VERSION, FIRST_COMMAND, notify_script):
  ...
  process_commands = [
    ...
    './scripts/write_process_nano_cmds.py --in_dir '+PICO_DIR+'/'+NANOAOD_VERSION+'/nano/'+YEAR+'/data/ --production '+PRODUCTION_NAME+' --dataset_list txt/datasets/'+NANOAOD_VERSION+'_bbgg_higgsino_'+YEAR+'_data_dataset_paths --data --tag '+data_tag,
...
''', formatter_class=argparse.RawTextHelpFormatter)
  ...
  '''
  slim_rule_files = [
    'txt/slim_rules/bbggmc.txt',
    'txt/slim_rules/bbggdata.txt',
    ]
  for slim_rule_file in slim_rule_files:
    if not os.path.exists(slim_rule_file): 
      print('[Error] '+slim_rule_file+' does not exist. Exiting.')
      sys.exit()
  '''
  ...
  if not os.path.exists(args.base_foldername+'/'+args.nanoaod_version+'/'+args.tag_name):
    ...
    #os.makedirs('/net/cms17/cms17r0/pico' + '/'+args.nanoaod_version+'/'+args.tag_name) 
  ...
  #PICO_DIR = '/net/cms17/cms17r0/pico'
