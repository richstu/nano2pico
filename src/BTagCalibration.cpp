  ...
  tagger_(taggr)
{
  ...
  if (!ifs.good()) {
    ERROR(("BTagCalibration: input file not available: "+filename));
  }
  readCSV(ifs);
  ifs.close();
}
  ...
  const BTagEntry::Parameters &par) const
{
  ...
  if (!data_.count(tok)) {
    ERROR(("BTagCalibration: (OperatingPoint, measurementType, sysType) not available: "+tok));
  }
  return data_.at(tok);
}
...
{
  std::stringstream buff;
  buff << static_cast<unsigned int>(par.operatingPoint) << ", "
       << par.measurementType << ", "
       << par.sysType;
  return buff.str();
}
