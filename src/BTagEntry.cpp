...
BTagEntry::BTagEntry(const std::string &csvLine)
{
  ...
  if (vec.size() != 11) {
    ERROR(("BTagCalibration: Invalid csv line; num tokens != 11: "+csvLine));
  }
  ...
  if (f1.IsZombie()) {
    ERROR(("BTagCalibration: Invalid csv line; formula does not compile: "+csvLine));
  }

  // make parameters
  unsigned op = stoi(vec[0]);
  if (op > 3) {
    ERROR(("BTagCalibration: Invalid csv line; OperatingPoint > 3: "+csvLine));
  }
  unsigned jf = stoi(vec[3]);
  if (jf > 2) {
    ERROR(("BTagCalibration: Invalid csv line; JetFlavor > 2: "+csvLine));
  }
  ...
  params(p)
{
  TF1 f1("", formula.c_str());  // compile formula to check validity
  if (f1.IsZombie()) {
    ERROR(("BTagCalibration: Invalid func string; formula does not compile: "+func));
  }
}
...
{
  if (func->IsZombie()) {
    ERROR(("BTagCalibration: Invalid TF1 function; function is zombie: "+std::string(func->GetName())));
  }
}
...
std::string th1ToFormulaBinTree(const TH1* hist, int start=0, int end=-1) {
  if (end == -1) {                      // initialize
    ...
    TH1* h2 = static_cast<TH1*>(hist->Clone());
  ...
  params(p)
{
  ...
  if (f1.IsZombie()) {
    ERROR(("BTagCalibration: Invalid histogram; formula does not compile (>150 bins?): "+std::string(hist->GetName())));
  }
}
...
{
  std::stringstream buff;
  buff << static_cast<unsigned>(params.operatingPoint)
       << ", " << params.measurementType
       << ", " << params.sysType
       << ", " << static_cast<unsigned>(params.jetFlavor)
       << ", " << params.etaMin
       << ", " << params.etaMax
       << ", " << params.ptMin
       << ", " << params.ptMax
